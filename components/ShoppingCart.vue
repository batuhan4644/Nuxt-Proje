
<template>
  <section class="pattern-group pattern-group-t-0 pattern-group-p-product">
    <div class="container">
      <div class="pattern-group-body">
        <div class="row">
          <div
            class="col-list col-12 d-block col-sm-12 d-sm-block col-md-12 d-md-block col-lg-12 d-lg-block col-xl-12 d-xl-block p-g-b-c-0"
          >
            <div class="p-g-b-c-wrapper">
              <div class="p-g-b-c-inner">
                <div
                  class="p-g-mod p-g-mod-t-13 p-g-mod-base-content p-g-mod-trans"
                >
                  <div class="p-g-mod-body p-g-mod-body-p-0">
                    <div class="product-profile-1" data-product-id="3601">
                      <div class="row">
                        <div class="col-lg-5">
                          <div
                            id="product-profile-carousel-3601"
                            class="carousel slide"
                            data-ride="carousel"
                          >
                            <div class="carousel-outer">
                              <div class="carousel-inner">
                                <div class="carousel-item active">
                                  <a
                                    href="/sepetimage"
                                    data-fancybox="images"
                                    data-caption="Renkli Ahşap Denge Oyunu"
                                    ><img
                                      src="../assets/sepeturun1.jpg"
                                      id="zoom_01"
                                      data-zoom-image="../assets/sepeturun1.jpg"
                                      alt="Renkli Ahşap Denge Oyunu"
                                  /></a>
                                </div>
                              </div>
                            </div>
                            <ol class="carousel-indicators">
                              <li
                                data-target="#product-profile-carousel-3601"
                                data-slide-to="0"
                                class="active"
                              >
                                <img
                                  class="d-block w-100"
                                  src="../assets/sepeturunkucuk1.jpg"
                                  alt="Renkli Ahşap Denge Oyunu"
                                />
                              </li>
                            </ol>
                          </div>
                        </div>
                        <div class="col-lg-7">
                          <h1 class="title">Renkli Ahşap Denge Oyunu</h1>
                          <ul class="product-profile-info">
                            <li>
                             <div class="product-reviews">
                                    <img height="20" width="60" src="../assets/Yıldız.PNG" />
                                
                              </div>
                            </li>
                            <li>
                              Ürün Kodu: <span class="value">SW7XB896PN</span>
                            </li>
                            <li>
                              Kategori:
                              <a
                                href="/otomotiv-yapi-market/yapi-market-c-174"
                                alt="Yapı Market"
                                ><span class="value">Yapı Market</span></a
                              >
                            </li>
                            <li>
                              Stok:
                              <span class="value"
                                ><span class="text-success">20</span></span
                              >
                            </li>
                            <li>Fiyat</li>
                            <li>
                              <div class="product-price-group">
                                <div class="discount">
                                  <span class="rate">%43</span
                                  ><span>indirim</span>
                                </div>
                                <div class="prices">
                                  <div class="list-price">69,00 TL</div>
                                  <div class="sale-price sale-variant-price">
                                    39,00 TL
                                  </div>
                                </div>
                              </div>
                            </li>
                          </ul>
                          <div class="product-buttons">
                            <div class="product-quantity">
                              <a
                                href="javascript:;"
                                class="btn btn-minus"
                                onclick="productQuantityBox(
                                    'decrement',
                                    'div[data-product-id=\'3601\'] .product-quantity'
                                )"
                                ><img height="20" width="20 " src="../assets/Eksi.PNG" /></a
                              ><input
                                class="form-control"
                                name="quantity"
                                min="1"
                                max="9999"
                                maxlength="4"
                                value="1"
                                onkeypress="return productQuantityBox(
                                    'change',
                                    'div[data-product-id=\'3601\'] .product-quantity',
                                    event
                                )"
                              /><a
                                href="javascript:;"
                                class="btn btn-plus"
                                onclick="productQuantityBox(
                                    'increment',
                                    'div[data-product-id=\'3601\'] .product-quantity'
                                )"
                                ><img height="20" width="20 " src="../assets/Artı.PNG" /></a>
                            </div>
                            <a
                              href="/sepet"
                              class="btn btn-fast-buy btn-color-1"
                              target="_blank"
                              ><img height="20" width="20 " src="../assets/SepetEkle.PNG" />
                              Sepete Ekle
                            </a> <a
                              href="/sepet"
                              class="btn btn-fast-buy btn-color-2"
                              target="_blank"
                              ><img height="20" width="20 " src="../assets/tl.PNG" />
                              Hemen Al
                            </a>
                            <a
                              href="/"
                              class="btn btn-wp"
                              target="_blank"
                              ><img height="20" width="20 " src="../assets/Whatsapp.PNG" />
                              WHATSAPP İLE SİPARİŞ VER
                            </a>
                          </div>
                          <div
                            class="product-favourite"
                            data-favourite-product-id="3601"
                          >
                            <a
                              href="/sepet"
                              class="d-block add-favorite"
                              onclick="userProductFavourite('add', 3601)"
                              ><img  src="../assets/urundetayimages/kalp.PNG" />
                              Favorilerime ekle </a
                            ><a
                              href="javascript:;"
                              class="d-none remove-favorite"
                              onclick="userProductFavourite('remove', 3601)"
                              ><img  src="../assets/urundetayimages/iade1.PNG" />
                              Favorilerimden çıkar
                            </a>
                          </div>
                          <div class="product-badges">
                            <div class="p-badge shipping-fast">
                             <img  src="../assets/urundetayimages/hizligonderim1.PNG" />
                             <span>Hızlı Gönderi</span>
                            </div>
                            <div class="p-badge door-payment">
                              <img  src="../assets/urundetayimages/guvenlialısveris1.PNG" />
                              <span>Güvenli Alışveriş</span>
                            </div>
                            <div class="p-badge credit-card">
                              <img  src="../assets/urundetayimages/iade1.PNG" />
                              <span>İade ve Değişim</span>
                            </div>
                          </div>
                          <div class="social-share jssocials">
                            <div class="jssocials-shares">
                              <div
                                class="jssocials-share jssocials-share-twitter"
                              >
                                <a
                                  target="_blank"
                                  href="/"
                                  class="jssocials-share-link"
                                  ><img height="20" width="20 " src="../assets/Twitter.PNG" /></a>
                              </div>
                              <div
                                class="jssocials-share jssocials-share-facebook"
                              >
                                <a
                                  target="_blank"
                                  href="/"
                                  class="jssocials-share-link"
                                  ><img height="20" width="20 " src="../assets/Facebook.PNG" /></a>
                              </div>
                              <div
                                class="jssocials-share jssocials-share-linkedin"
                              >
                                <a
                                  target="_blank"
                                  href="/"
                                  class="jssocials-share-link"
                                  ><img height="20" width="20 " src="../assets/Linkedin.PNG" /></a>
                              </div>
                              <div
                                class="jssocials-share jssocials-share-whatsapp"
                              >
                                <a
                                  target="_self"
                                  href="/"
                                  ><img height="20" width="20 " src="../assets/Whatsapp.PNG" /></a>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

